...
#if defined(NDEBUG)
# error "Sealandcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5772e46e9d7dd3712067aabec7765d7d161d06120c637c64ad538db15b9aed54");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sealandcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sealandcoin Signed Message:\n";
...
{
    // Sealandcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 2 * 30; // Sealandcoin: 1 Minute
static const int64 nTargetSpacing = 1 * 30; // Sealandcoin: 30 Seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sealandcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sealandcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5772e46e9d7dd3712067aabec7765d7d161d06120c637c64ad538db15b9aed54");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 08/01/2017 Sealand Granted Full Membership Status At The U.N.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x71a6cd0157fccee7f243aab5a8a2bde9f3febe30f80913f48ea90d5c2d88e6b6"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Sealandcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("sealandcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("sealandcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static sealandcoinMiner(CWallet *pwallet)
{
    printf("sealandcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sealandcoin-miner");
    ...
    try { loop {
        ...
        printf("Running sealandcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("sealandcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&sealandcoinMiner, pwallet));
}
